...
On the τ²-Bench retail held-out slice the agent scores `[ABLATION.B_MECHANISM.PASS_AT_1]` ± `[ABLATION.B_MECHANISM.CI_95]` at `$[ABLATION.B_MECHANISM.COST_PER_TASK]` per task, versus `[ABLATION.A_BASELINE.PASS_AT_1]` baseline and `[GEPA.PASS_AT_1]` for GEPA at the same compute budget.
...
| Day-1 baseline | `[ABLATION.A_BASELINE.PASS_AT_1]` | `[ABLATION.A_BASELINE.CI_95]` | `[ABLATION.A_BASELINE.COST_PER_TASK]` | `[ABLATION.A_BASELINE.P95]` |
| **Our mechanism** | `[ABLATION.B_MECHANISM.PASS_AT_1]` | `[ABLATION.B_MECHANISM.CI_95]` | `[ABLATION.B_MECHANISM.COST_PER_TASK]` | `[ABLATION.B_MECHANISM.P95]` |
| GEPA (same budget) | `[GEPA.PASS_AT_1]` | `[GEPA.CI_95]` | `[GEPA.COST_PER_TASK]` | — |

Delta A (mechanism − baseline) = `[DELTAS.DELTA_A]` with p=`[DELTAS.DELTA_A_P_VALUE]`, 95% CI `[DELTAS.DELTA_A_CI_95]`.

### Cost per qualified lead

- LLM + rig spend: `$[PROD.SPEND_TOTAL]` over `[PROD.DAYS]` days.
- Qualified leads (classifier ≥ 0.6 AND reply classified engaged/curious): `[PROD.QUALIFIED_COUNT]`.
- CPL: **$[PROD.CPL]** vs. target $5, penalty at $8.

### Speed-to-lead delta

- Tenacious manual stalled-thread baseline (seed/baseline_numbers.md): 30–40%.
- Our system: `[PROD.STALLED_RATE_PCT]%` on `[PROD.N_THREADS]` measured threads.
- Sample-size caveat: `[PROD.SAMPLE_NOTE]`.

### Competitive-gap outbound performance

- `[PROD.SIGNAL_GROUNDED_FRACTION]%` of outbound led with a research finding
  (AI-maturity + top-quartile competitor-gap evidence) vs. generic pitch.
- Reply-rate delta (grounded − generic): **+`[PROD.REPLY_DELTA_PCT]`%**.
...
`[METHOD.P0402_RATE]%`. If deployed without isolation, a Segment 1 draft
...
*Footer, every page: page N of 2 · generated `[RENDER_TIMESTAMP_UTC]` ·
`evidence_graph.json` sha256 `[EVIDENCE_GRAPH_HASH]`.*
